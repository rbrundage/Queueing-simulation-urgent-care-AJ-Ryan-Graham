...
%[text] ## Part 1: Probabilities and Computation
% Probabilities
P0 = 1 - rho;
P1 = P0 * rho;
P2 = P0 * rho^2;
...
W  = L / lambda;
% Print
fprintf("L = %.4f customers\n", L);
fprintf("Lq = %.4f customers\n", Lq);

fprintf("W = %.4f hours = %.2f minutes\n", W, 60*W);
fprintf("Wq = %.4f hours = %.2f minutes\n", Wq, 60*Wq);
%[text] ## Other Histograms:
% the count of customers waiting
NumWaitingSamples = cellfun(@(q) q.Log.NumWaiting, QSamples, 'UniformOutput', false);
NumWaiting = vertcat(NumWaitingSamples{:});
figure; histogram(NumWaiting);
...
for SampleNum = 1:NumSamples
    q = QSamples{SampleNum};
    nServed = length(q.Served);
    waits = zeros(nServed,1);

    for i = 1:nServed
        c = q.Served{i};

        if i == 1
            serviceStart = c.ArrivalTime;
        else
            prev = q.Served{i-1};
            serviceStart = max(c.ArrivalTime, prev.DepartureTime);
        end

        waits(i) = serviceStart - c.ArrivalTime;
    end

    TimeWaitingSamples{SampleNum} = waits;
end

TimeWaiting = vertcat(TimeWaitingSamples{:});
figure; histogram(TimeWaiting);
title("Time in queue");
...
for SampleNum = 1:NumSamples
    ...
    for i = 1:nServed
        ...
        serviceTimes(i) = c.DepartureTime - serviceStart;
    end

    ServiceTimeSamples{SampleNum} = serviceTimes;
end

ServiceTime = vertcat(ServiceTimeSamples{:});
figure; histogram(ServiceTime);
title("Service time");

% the count of customers served per shift
ServedPerShift = cellfun(@(q) length(q.Served), QSamples);
figure; histogram(ServedPerShift);
title("Customers Served Per shift");
...
fprintf("L simulation = %.4f, theory = %.4f\n", L_sim, L);
fprintf("Lq simulation = %.4f, theory = %.4f\n", Lq_sim, Lq);
fprintf("W simulation = %.4f hours = %.2f minutes, theory = %.4f hours = %.2f minutes\n", ...
