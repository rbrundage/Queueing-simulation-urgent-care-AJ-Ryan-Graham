classdef ServiceQueueRenege < handle
    ...
    properties (SetAccess = public)
        ...
        LogInterval = 1/60;
      % RenegeRate - Rate parameter theta for customer patience.
        RenegeRate = 4; %15 Minutes = 4 per Hour
    end
    ...
    properties (SetAccess = private)
        ...
        ServiceDist;
          ...
          % RenegeDist - Distribution object for time until a waiting
        % customer reneges.
        RenegeDist;
        ...
        Served = {};
          ...
          % Reneged - Cell array row vector of Customer objects who left
        % before being served.
        Reneged = {};
    ...
    methods
        ...
        function handle_arrival(obj, arrival)
            ...
            % Reneging event in case this customer waits too long.
            renege_time = random(obj.RenegeDist);
            next_renege = Renege(obj.Time + renege_time, c);
            schedule_event(obj, next_renege);
        ...
        function handle_renege(obj, renege)
            % handle_renege Handle a reneging event.

            % Look through the waiting line for the customer tied to event.
            for j = 1:length(obj.Waiting)
                customer = obj.Waiting{j};

                if customer.Id == renege.Customer.Id
                    % Add this Customer object to the end of Reneged.
                    obj.Reneged{end+1} = customer;

                    % Remove this customer from Waiting.
                    obj.Waiting(j) = [];
            ...
            % If the customer is not in Waiting, do nothing. 
        end
