...
mkdir(PictureFolder); %[output:5ae1b8f8]
%%
%[text] ## Set up
...
LogInterval = 1/60;
%%
%[text] ## Eq Dist for M/M/1 queue with reneging
...
lambda = 2;     % arrivals per hour
mu = 3;         % service rate per hour
theta = 4;      % reneging rate per hour (mean wait = 15 min = 0.25 hr, so theta = 1/0.25 = 4)

% M/M/1 with reneging Probs
...
for SampleNum = 1:NumSamples %[output:group:9fba91d5]
    if mod(SampleNum, 10) == 0
        fprintf("%d ", SampleNum); %[output:193fa6cc]
    end
...
fprintf("Mean number in system: %f\n", meanNumInSystem); %[output:7337495a]
%[text] Make a figure with one set of axes.
fig = figure(); %[output:7c299695]
t = tiledlayout(fig,1,1); %[output:7c299695]
ax = nexttile(t); %[output:7c299695]
...
h = histogram(ax, NumInSystem, Normalization="probability", BinMethod="integers"); %[output:7c299695]
%[text] Plot $(0, P\_0), (1, P\_1), \\dots$.  If all goes well, these dots should land close to the tops of the bars of the histogram.
plot(ax, 0:NMax, P, 'o', MarkerEdgeColor='k', MarkerFaceColor='r'); %[output:7c299695]
%[text] Add titles and labels and such.
title(ax, "Number of customers in the system"); %[output:7c299695]
xlabel(ax, "Count"); %[output:7c299695]
ylabel(ax, "Probability"); %[output:7c299695]
legend(ax, "simulation", "theory"); %[output:7c299695]
...
exportgraphics(fig, PictureFolder + filesep + "Number in system histogram.pdf"); %[output:7c299695]
exportgraphics(fig, PictureFolder + filesep + "Number in system histogram.svg"); %[output:7c299695]
%%
...
fprintf("Mean time in system: %f\n", meanTimeInSystem); %[output:897deb48]
%[text] Make a figure with one set of axes.
fig = figure(); %[output:64e71e2f]
t = tiledlayout(fig,1,1); %[output:64e71e2f]
ax = nexttile(t); %[output:64e71e2f]
...
h = histogram(ax, TimeInSystem, Normalization="probability", BinWidth=5/60); %[output:64e71e2f]
%[text] Add titles and labels and such.
title(ax, "Time in the system"); %[output:64e71e2f]
xlabel(ax, "Time"); %[output:64e71e2f]
ylabel(ax, "Probability"); %[output:64e71e2f]
%[text] Set ranges on the axes.
ylim(ax, [0, 0.2]); %[output:64e71e2f]
xlim(ax, [0, 2.0]); %[output:64e71e2f]
%[text] Wait for MATLAB to catch up.
pause(2);
%[text] Save the picture.
exportgraphics(fig, PictureFolder + filesep + "Time in system histogram.pdf"); %[output:64e71e2f]
exportgraphics(fig, PictureFolder + filesep + "Time in system histogram.svg"); %[output:64e71e2f]
%[text] ### 3.2 Fraction of customers served (no reneging)

pi_s = mu * (1 - P0) / lambda;

fprintf("pi_s (fraction served) = %.6f\n", pi_s); %[output:4a89759d]
%[text] ## 
...
%---
%[output:0e854850]
%   data: {"dataType":"text","outputData":{"text":"P0 = 0.527169\n","truncated":false}}
%---
%[output:55fb240a]
%   data: {"dataType":"text","outputData":{"text":"P1 = 0.351446\n","truncated":false}}
%---
%[output:6458e198]
%   data: {"dataType":"text","outputData":{"text":"P2 = 0.100413\n","truncated":false}}
%---
%[output:9f2692c8]
%   data: {"dataType":"text","outputData":{"text":"P3 = 0.018257\n","truncated":false}}
%---
%[output:901b7fa1]
%   data: {"dataType":"text","outputData":{"text":"P4 = 0.002434\n","truncated":false}}
%---
%[output:286ac50b]
%   data: {"dataType":"text","outputData":{"text":"P5 = 0.000256\n","truncated":false}}
%---
%[output:193fa6cc]
...
%---
%[output:4a89759d]
%   data: {"dataType":"text","outputData":{"text":"pi_s (fraction served) = 0.709246\n","truncated":false}}
%---
